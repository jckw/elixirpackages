defmodule HexPackagesWeb.Admin.PackageControllerTest do
  ...
  describe "create package" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, Routes.admin_package_path(conn, :create), package: @create_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, Routes.admin_package_path(conn, :create), package: @invalid_attrs
      assert html_response(conn, 200) =~ "New Package"
    end
  end
  ...
  describe "update package" do
    ...
    test "renders errors when data is invalid", %{conn: conn, package: package} do
      conn = put conn, Routes.admin_package_path(conn, :update, package), package: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Package"
    end
  end
